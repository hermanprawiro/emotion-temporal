...
class Enterface_Sparse(SparseDataset):
    ...
    def _parse_list(self):
        ...
        annot_path = 'datasets/lists/enterface/enterface_%s_split%02d.csv' % ('train' if self.train else 'test', self.which_split)
        video_info = pd.read_csv(annot_path, header=None)

        for _, row in video_info.iterrows():
            vpath, vlabel, vlen = row
    ...
    def _parse_class_names(self):
        ...
        class_desc_file = 'datasets/lists/enterface/enterface_classInd.txt'
