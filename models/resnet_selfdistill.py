...
def get_model(arch='resnet50', pretrained=True):
    if arch == 'resnet18':
        ...
        net = wide_resnet50_2(pretrained=pretrained)
    elif arch == 'resnext50':
        net = resnext50_32x4d(pretrained=pretrained)
    else:
        raise NotImplementedError
...
class Network(nn.Module):
    def __init__(self, backbone='resnet50', pretrained=True, n_class=6):
        super().__init__()

        self.net = get_model(arch=backbone, pretrained=pretrained)
        self.conv_dim = get_feature_dim(arch=backbone)

        self.pool1 = nn.Sequential(
            nn.Conv3d(self.conv_dim // 8, self.conv_dim // 8, 3, stride=1, padding=1),
            nn.ReLU(),
            nn.AdaptiveMaxPool3d((1, None, None))
        )
        self.pool2 = nn.Sequential(
            nn.Conv3d(self.conv_dim // 4, self.conv_dim // 4, 3, stride=1, padding=1),
            nn.ReLU(),
            nn.AdaptiveMaxPool3d((1, None, None))
        )
        self.pool3 = nn.Sequential(
            nn.Conv3d(self.conv_dim // 2, self.conv_dim // 2, 3, stride=1, padding=1),
            nn.ReLU(),
            nn.AdaptiveMaxPool3d((1, None, None))
        )
        ...
        self.net.fc1 = nn.Linear(self.conv_dim, n_class)
        self.net.fc2 = nn.Linear(self.conv_dim, n_class)
        self.net.fc3 = nn.Linear(self.conv_dim, n_class)
        self.net.fc4 = nn.Linear(self.conv_dim, n_class)
    ...
    def forward(self, x):
        ...
        f1, f2, f3, f4 = self.net(x)

        f1 = f1.view((N, T) + f1.shape[1:]).permute(0, 2, 1, 3, 4)
        f1 = self.pool1(f1).squeeze(2)
        f1 = self.net.scala1(f1).flatten(1)
        out1 = self.net.fc1(f1)

        f2 = f2.view((N, T) + f2.shape[1:]).permute(0, 2, 1, 3, 4)
        f2 = self.pool2(f2).squeeze(2)
        f2 = self.net.scala2(f2).flatten(1)
        out2 = self.net.fc2(f2)

        f3 = f3.view((N, T) + f3.shape[1:]).permute(0, 2, 1, 3, 4)
        f3 = self.pool3(f3).squeeze(2)
        f3 = self.net.scala3(f3).flatten(1)
        out3 = self.net.fc3(f3)

        f4 = f4.view((N, T) + f4.shape[1:]).permute(0, 2, 1, 3, 4)
        f4 = self.pool4(f4).squeeze(2)
        f4 = self.net.scala4(f4).flatten(1)
        out4 = self.net.fc4(f4)

        return [out4, out3, out2, out1], [f4, f3, f2, f1]
