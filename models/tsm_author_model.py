...
class TSN(nn.Module):
    ...
    def _get_diff(self, input, keep_rgb=False):
        input_c = 3 if self.modality in ["RGB", "RGBDiff"] else 1
...
class Network(nn.Module):
    def __init__(self, pretrained=True, n_class=10):
        ...
        self.net.new_fc = nn.Linear(self.conv_dim, n_class)
    ...
    def modify_input_channels(self, new_input_channel):
        modules = list(self.net.base_model.modules())
        first_conv_idx = list(filter(lambda x: isinstance(modules[x], nn.Conv2d), list(range(len(modules)))))[0]
...
if __name__ == "__main__":
    net = Network('gray')
    print(net(torch.randn(1, 1, 16, 112, 112)).shape)