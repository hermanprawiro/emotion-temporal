...
from models.resnet_selfdistill import Network
...
def main(args):
    ...
    save_name = "selfdistill_sparse_{}_{}_{:d}seg_split{:d}_bs{}_wd{}".format(args.backbone, args.dataset, args.n_segments, args.split, args.batch_size, args.weight_decay)
    if args.comment != '':
    ...
    net = Network(backbone=args.backbone).to(device)
...
def train(loader, model, criterion, optimizer, epoch, writer=None):
    ...
    for i, (inputs, labels) in enumerate(loader):
        ...
        for idx in range(1, len(outputs)):
            # logits distillation
            loss += CrossEntropy(outputs[idx], teacher_output) * loss_coefficient
            loss += criterion(outputs[idx], labels) * (1 - loss_coefficient)

            # feature distillation
            if idx == len(outputs) - 1: # won't be applied to shallowest classifier
                loss += torch.dist(model.module.net.adaptation_layers[idx - 1](outputs_feature[idx]), teacher_feature) * feature_loss_coefficient

        # measure accuracy and record loss
        prec1, prec5 = misc.accuracy(ensemble, labels, topk=(1, 5))
...
def validate(loader, model, criterion, epoch=0, writer=None, show_confusion=False):
    ...
    top1 = misc.AverageMeter()
    top5 = misc.AverageMeter()
    top1_layer1 = misc.AverageMeter()
    top1_layer2 = misc.AverageMeter()
    top1_layer3 = misc.AverageMeter()
    top1_layer4 = misc.AverageMeter()
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(loader):
            ...
            if show_confusion:
                y_preds.append(ensemble.argmax(dim=1).cpu().numpy())
            ...
            top1.update(prec1[0], n_batch)
            top5.update(prec5[0], n_batch)
            prec1, _ = misc.accuracy(outputs[0], labels, topk=(1, 5))
            top1_layer4.update(prec1[0], n_batch)
            prec1, _ = misc.accuracy(outputs[1], labels, topk=(1, 5))
            top1_layer3.update(prec1[0], n_batch)
            prec1, _ = misc.accuracy(outputs[2], labels, topk=(1, 5))
            top1_layer2.update(prec1[0], n_batch)
            prec1, _ = misc.accuracy(outputs[3], labels, topk=(1, 5))
            top1_layer1.update(prec1[0], n_batch)
            ...
            if i % 5 == 4:
                print('Test: [{0}/{1}]\t'
                    ...
                    # 'Data {data_time.val:.3f} ({data_time.avg:.3f})\t'
                    'Loss {loss.val:.4f} ({loss.avg:.4f})\t'
                    'Acc 4/4 {top1_layer4.val:.3f} ({top1_layer4.avg:.3f})\t'
                    'Acc 3/4 {top1_layer3.val:.3f} ({top1_layer3.avg:.3f})\t'
                    'Acc 2/4 {top1_layer2.val:.3f} ({top1_layer2.avg:.3f})\t'
                    'Acc 1/4 {top1_layer1.val:.3f} ({top1_layer1.avg:.3f})\t'
                    'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
                    'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                        i+1, len(loader), 
                        # batch_time=batch_time, data_time=data_time, 
                        top1_layer1=top1_layer1, top1_layer2=top1_layer2,
                        top1_layer3=top1_layer3, top1_layer4=top1_layer4,
                        loss=losses, top1=top1, top5=top5))

        print(' * 4/4 {top1_layer4.avg:.3f}\t'
            '3/4 {top1_layer3.avg:.3f}\t'
            '2/4 {top1_layer2.avg:.3f}\t'
            '1/4 {top1_layer1.avg:.3f}\t'
            'Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f}'
              .format(top1_layer1=top1_layer1, top1_layer2=top1_layer2,
                    top1_layer3=top1_layer3, top1_layer4=top1_layer4,
                    top1=top1, top5=top5))
