...
def main(args):
    ...
    checkpoint = torch.load("pretrained/bp4d_r3d_16seg_split1/checkpoint_best.pth")['state_dict']
    del(checkpoint['net.fc.weight'])
    del(checkpoint['net.fc.bias'])
    net.load_state_dict(checkpoint, strict=False)

    # if args.split == 1:
    #     lbl_weights = torch.tensor([56, 15, 48, 71, 65, 87, 62], dtype=torch.float32, device=device)
    # elif args.split == 2:
    #     lbl_weights = torch.tensor([66, 15, 51, 64, 66, 78, 54], dtype=torch.float32, device=device)
    # elif args.split == 3:
    #     lbl_weights = torch.tensor([58, 15, 49, 64, 63, 89, 58], dtype=torch.float32, device=device)
    # elif args.split == 4:
    #     lbl_weights = torch.tensor([54, 16, 48, 62, 63, 79, 52], dtype=torch.float32, device=device)
    # elif args.split == 5:
    #     lbl_weights = torch.tensor([62, 15, 40, 63, 63, 79, 66], dtype=torch.float32, device=device)
    # lbl_weights = 1 / lbl_weights
