...
def main(args):
    # checkpoint_name = "{}_ctbc_{:d}seg_split{:d}_bs16_wd0.001".format(args.arch, args.n_segments, args.split)
    checkpoint_name = "{}_ctbc2_{:d}seg_split{:d}_bs16_wd0.001_{}_allcam".format(args.arch, args.n_segments, args.split, "static" if args.static else "dynamic")
    ...
    net = Network(arch=args.arch, n_class=7).to(device)
    ...
    for emotion in os.listdir(args.root_path):
        ...
        for clip_name in os.listdir(path_with_emotion):
            ...
            for i, inputs in enumerate(loader):
                ...
                labels = torch.LongTensor([labels_idx]).repeat(n_batch * args.n_segments).to(device)
                ...
                original_inputs = original_inputs.permute(0, 2, 1, 3, 4).reshape(n_batch * args.n_segments, 3, args.image_res, args.image_res)
                original_inputs = F.interpolate(original_inputs, (256, 256))

                results = original_inputs * 0.7
                results[:1] = put_text(results[:1], outputs, labels, class_dict_decode)

                torchvision.utils.save_image(results, os.path.join(args.result_path, "%s_%s_%04d.jpg" % (emotion, clip_name, i)), normalize=False, nrow=8)
